...
# Tuned for 4 GPUs by default.
TRAIN_BATCH_SIZE=48
PPO_MIMI_BATCH_SIZE=48
ROLLOUT_N=8
...
# --- HuggingFace cache (models download to workspace, not $HOME) ---
...
# --- Data Paths ---
DATA_DIR="${DATA_DIR:-${PROJECT_ROOT}/../cache/opd-v}"
TASK_TRAIN_FILE="${DATA_DIR}/train.parquet"

MODEL_NAME="${MODEL_NAME:-$(basename "$MODEL_PATH")}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-OPD-V-${MODEL_NAME}}"
PROJECT_NAME="${PROJECT_NAME:-opd-v}"
OPDV_WORK_DIR="${OPDV_WORK_DIR:-${PROJECT_ROOT}/../cache/opd-v}"
TRAINER_DEFAULT_LOCAL_DIR="${OPDV_WORK_DIR}/checkpoints/${EXPERIMENT_NAME}"
TRAINER_ROLLOUT_DATA_DIR="${OPDV_WORK_DIR}/rollouts/${EXPERIMENT_NAME}"

# Set WANDB_MODE=offline on compute nodes without outbound network, then: wandb sync ...
: "${WANDB_MODE:=online}"
export WANDB_MODE
export WANDB_DIR="${WANDB_DIR:-${OPDV_WORK_DIR}/wandb_logs}"
...
mkdir -p "$WANDB_DIR" "$WANDB_CACHE_DIR" "$WANDB_CONFIG_DIR" \
         "$TRAINER_ROLLOUT_DATA_DIR" "${TRAINER_DEFAULT_LOCAL_DIR}"
...
echo "Train data: $TASK_TRAIN_FILE"
echo "OPDV_WORK_DIR: $OPDV_WORK_DIR"
echo "Checkpoints: $TRAINER_DEFAULT_LOCAL_DIR"
echo "Rollouts: $TRAINER_ROLLOUT_DATA_DIR"
echo "W&B project: $PROJECT_NAME (mode=$WANDB_MODE, dir=$WANDB_DIR)"
