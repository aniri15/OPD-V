"""Download and preprocess training data from Hugging Face.

Usage:
    python scripts/prepare_data.py --data-dir ./data --hf-repo <dataset_repo>

This script:
1. Downloads train.jsonl from the selected Hugging Face dataset repo
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare OPD-V training data.")
    ...
    if not args.skip_download and not args.hf_repo:
...
def download_dataset(repo_id: str, data_dir: str) -> None:
    ...
    if tar_files:
        ...
        subprocess.run(f"cat {' '.join(tar_files)} | tar -xzf - -C .", shell=True, cwd=images_dir, check=True)
        for f in tar_files:
    ...
    if os.path.exists(teacher_tar):
        ...
        subprocess.run(["tar", "-xzf", "teacher_images.tar.gz", "-C", "."], cwd=teacher_dir, check=True)
