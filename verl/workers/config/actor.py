...
class SelfDistillationConfig(BaseConfig):
    ...
    Args:
        ...
        contrastive_visual_advantage (bool): If True, compute a positive-vs-negative teacher token log-prob
            advantage and use it to weight the positive-teacher distillation loss.
        contrastive_negative_mode (str): Visual perturbation used for the negative teacher.
        contrastive_patch_mask_ratio (float): Patch masking probability for patch-based-mask negative teacher.
        contrastive_patch_size (int): Patch side length in pixels for patch-based-mask negative teacher.
        post_encoder_visual_token_drop_rate (float): Visual token dropout probability applied after the
            negative teacher vision encoder when contrastive_negative_mode="post-encoder-token-drop".
        input_visual_token_drop_rate (float): Visual token deletion probability applied to Qwen3.5/Qwen3-VL
            visual token positions after embeddings are built and before the LLM when
            contrastive_negative_mode="input-visual-token-drop".
        ...
        analysis_dump_dir (Optional[str]): Optional directory used to dump per-token visual advantage analysis
            for contrastive teacher runs.
    """
    ...
    contrastive_negative_mode: str = "no-image"
    contrastive_patch_mask_ratio: float = 0.6
    contrastive_patch_size: int = 14
    post_encoder_visual_token_drop_rate: float = 0.3
    input_visual_token_drop_rate: float = 0.3
    ...
    analysis_dump_dir: Optional[str] = None
    ...
    def __post_init__(self):
        ...
        if self.teacher_extra_student_image_blocks < 0:
            raise ValueError(
        ...
        valid_contrastive_negative_modes = [
            ...
            "random-mask",
            "patch-based-mask",
            "post-encoder-token-drop",
            "input-visual-token-drop",
        ]
        if self.contrastive_negative_mode not in valid_contrastive_negative_modes:
            raise ValueError(
                "self_distillation.contrastive_negative_mode must be one of "
                f"{valid_contrastive_negative_modes}, got {self.contrastive_negative_mode}"
            )
        if not 0.0 <= self.contrastive_patch_mask_ratio <= 1.0:
            raise ValueError(
                "self_distillation.contrastive_patch_mask_ratio must be in [0,1], "
                f"got {self.contrastive_patch_mask_ratio}"
            )
        if self.contrastive_patch_size <= 0:
            raise ValueError(
                "self_distillation.contrastive_patch_size must be a positive integer, "
                f"got {self.contrastive_patch_size}"
            )
        if not 0.0 <= self.post_encoder_visual_token_drop_rate <= 1.0:
            raise ValueError(
                "self_distillation.post_encoder_visual_token_drop_rate must be in [0,1], "
                f"got {self.post_encoder_visual_token_drop_rate}"
            )
        if not 0.0 <= self.input_visual_token_drop_rate <= 1.0:
            raise ValueError(
                "self_distillation.input_visual_token_drop_rate must be in [0,1], "
                f"got {self.input_visual_token_drop_rate}"
            )
