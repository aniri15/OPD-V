...
def _get_input_embeds(
    ...
    input_visual_token_keep_mask: Optional[torch.Tensor] = None,
):
    ...
    if input_visual_token_keep_mask is not None:
        input_visual_token_keep_mask = input_visual_token_keep_mask.to(device=inputs_embeds.device, dtype=torch.bool)
        if input_visual_token_keep_mask.shape != input_ids.shape:
            raise ValueError(
                "input_visual_token_keep_mask shape must match input_ids shape, "
                f"got {tuple(input_visual_token_keep_mask.shape)} vs {tuple(input_ids.shape)}"
            )

        kept_lengths = input_visual_token_keep_mask.sum(dim=1)
        if (kept_lengths <= 0).any():
            raise ValueError("input_visual_token_keep_mask cannot drop every token in a sequence")

        max_kept_len = int(kept_lengths.max().item())
        new_inputs_embeds = inputs_embeds.new_zeros((inputs_embeds.shape[0], max_kept_len, inputs_embeds.shape[-1]))
        new_attention_mask = None
        if attention_mask is not None:
            new_attention_mask = attention_mask.new_zeros((attention_mask.shape[0], max_kept_len))
        new_position_ids = None
        if position_ids is not None:
            if position_ids.dim() == 2:
                new_position_ids = position_ids.new_zeros((position_ids.shape[0], max_kept_len))
            elif position_ids.dim() == 3:
                new_position_ids = position_ids.new_zeros((position_ids.shape[0], position_ids.shape[1], max_kept_len))
            else:
                raise ValueError(f"Unsupported position_ids shape for visual token drop: {tuple(position_ids.shape)}")
        new_visual_pos_masks = None
        deepstack_keep_mask = None
        if visual_pos_masks is not None:
            visual_pos_masks = visual_pos_masks.to(device=inputs_embeds.device, dtype=torch.bool)
            deepstack_keep_mask = input_visual_token_keep_mask[visual_pos_masks]
            new_visual_pos_masks = visual_pos_masks.new_zeros((visual_pos_masks.shape[0], max_kept_len))

        for batch_idx in range(inputs_embeds.shape[0]):
            keep = input_visual_token_keep_mask[batch_idx]
            kept_len = int(kept_lengths[batch_idx].item())
            new_inputs_embeds[batch_idx, :kept_len] = inputs_embeds[batch_idx, keep]
            if new_attention_mask is not None:
                new_attention_mask[batch_idx, :kept_len] = attention_mask[batch_idx, keep]
            if new_position_ids is not None:
                if position_ids.dim() == 2:
                    new_position_ids[batch_idx, :kept_len] = position_ids[batch_idx, keep]
                elif position_ids.shape[0] == inputs_embeds.shape[0]:
                    new_position_ids[batch_idx, :, :kept_len] = position_ids[batch_idx, :, keep]
                else:
                    new_position_ids[:, batch_idx, :kept_len] = position_ids[:, batch_idx, keep]
            if new_visual_pos_masks is not None:
                new_visual_pos_masks[batch_idx, :kept_len] = visual_pos_masks[batch_idx, keep]

        if deepstack_visual_embeds is not None and deepstack_keep_mask is not None:
            deepstack_visual_embeds = [
                emb[deepstack_keep_mask.to(device=emb.device)] for emb in deepstack_visual_embeds
            ]

        inputs_embeds = new_inputs_embeds
        attention_mask = new_attention_mask
        position_ids = new_position_ids
        visual_pos_masks = new_visual_pos_masks
...
def qwen3_vl_base_forward(
    ...
    **kwargs,
    ...
    input_visual_token_keep_mask = kwargs.pop("input_visual_token_keep_mask", None)
