...
def compute_self_distillation_loss(
    ...
    if visual_advantage_weights is not None:
        visual_advantage_weights = visual_advantage_weights.to(dtype=weighted_per_token_loss.dtype)
        weighted_per_token_loss = weighted_per_token_loss * visual_advantage_weights
    ...
    if visual_advantage_weights is not None:
        metrics["self_distillation/visual_advantage_token_mean"] = (
            verl_F.masked_sum(visual_advantage_weights, loss_mask) / valid_token_count
        ).detach().item()
        metrics["self_distillation/visual_advantage_nonzero_fraction"] = (
            verl_F.masked_sum((visual_advantage_weights > 0).to(loss_mask.dtype), loss_mask) / valid_token_count
        ).detach().item()
        activated_mask = ((visual_advantage_weights > 0).to(loss_mask.dtype) * loss_mask)
        activated_token_count = activated_mask.sum().clamp(min=1.0)
        metrics["self_distillation/visual_advantage_activated_weight_mean"] = (
            verl_F.masked_sum(visual_advantage_weights, activated_mask) / activated_token_count
        ).detach().item()
        metrics["self_distillation/visual_advantage_bucket_zero_fraction"] = (
            verl_F.masked_sum((visual_advantage_weights <= 0).to(loss_mask.dtype), loss_mask) / valid_token_count
        ).detach().item()
        metrics["self_distillation/visual_advantage_bucket_small_fraction"] = (
            verl_F.masked_sum(
                ((visual_advantage_weights > 0) & (visual_advantage_weights <= 0.05)).to(loss_mask.dtype),
                loss_mask,
            )
            / valid_token_count
        ).detach().item()
        metrics["self_distillation/visual_advantage_bucket_medium_fraction"] = (
            verl_F.masked_sum(
                ((visual_advantage_weights > 0.05) & (visual_advantage_weights <= 0.2)).to(loss_mask.dtype),
                loss_mask,
            )
            / valid_token_count
        ).detach().item()
        metrics["self_distillation/visual_advantage_bucket_large_fraction"] = (
            verl_F.masked_sum((visual_advantage_weights > 0.2).to(loss_mask.dtype), loss_mask) / valid_token_count
        ).detach().item()
