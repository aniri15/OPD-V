...
class RayPPOTrainer:
    ...
    def _repeat_images_in_messages(self, messages: list[dict], extra_image_blocks: int) -> list[dict]:
        teacher_messages = deepcopy(messages)
        for message in teacher_messages:
            content = message.get("content")
            if not isinstance(content, list):
                ...
                if isinstance(item, dict) and item.get("type") == "image":
                    ...
                    if image is not None:
                        for _ in range(extra_image_blocks):
                            new_content.append({"type": "image", "image": self._normalize_teacher_image(image)})
            message["content"] = new_content
        return teacher_messages

    def _apply_contrastive_negative_to_messages(
        self,
        messages: list[dict],
        mode: str,
        patch_mask_ratio: float = 0.6,
        patch_size: int = 14,
    ) -> list[dict]:
        negative_messages = deepcopy(messages)
        for message in negative_messages:
            ...
            for item in content:
                ...
                if mode == "no-image":
                    continue
                if mode in {"post-encoder-token-drop", "input-visual-token-drop"}:
                    new_content.append(item)
                    continue
                ...
                image = self._perturb_teacher_image(
                    self._normalize_teacher_image(image),
                    mode,
    ...
    def _perturb_teacher_image(
        ...
        if mode == "random-blur":
            ...
            for _ in range(3):
                patch_w = int(rng.integers(max(1, width // 8), max(2, width // 2 + 1)))
                patch_h = int(rng.integers(max(1, height // 8), max(2, height // 2 + 1)))
                x0 = int(rng.integers(0, max(1, width - patch_w + 1)))
                y0 = int(rng.integers(0, max(1, height - patch_h + 1)))
                box = (x0, y0, x0 + patch_w, y0 + patch_h)
                output.paste(output.crop(box).filter(ImageFilter.GaussianBlur(radius=15)), box)
            return output
        if mode == "random-mask":
            output = image.copy()
            rng = np.random.default_rng(0)
            width, height = output.size
            target_area = max(1, int(width * height / 3.0))
            aspect = float(rng.uniform(0.5, 2.0))
            patch_w = min(width, max(1, int((target_area * aspect) ** 0.5)))
            patch_h = min(height, max(1, int(target_area / patch_w)))
            x0 = int(rng.integers(0, max(1, width - patch_w + 1)))
            y0 = int(rng.integers(0, max(1, height - patch_h + 1)))
            ImageDraw.Draw(output).rectangle((x0, y0, x0 + patch_w, y0 + patch_h), fill=(0, 0, 0))
            return output
        if mode == "patch-based-mask":
            output = image.copy()
            draw = ImageDraw.Draw(output)
            rng = np.random.default_rng()
            width, height = output.size
            patch_size = max(1, int(patch_size))
            patch_mask_ratio = min(1.0, max(0.0, float(patch_mask_ratio)))
            for y0 in range(0, height, patch_size):
                for x0 in range(0, width, patch_size):
                    if rng.random() < patch_mask_ratio:
    ...
    def _maybe_build_self_distillation_batch(
        ...
        if use_teacher_always_on_inputs:
            ...
            teacher_extra_student_image_blocks = self_distillation_cfg.get(
                "teacher_extra_student_image_blocks", 0
            )
            contrastive_visual_advantage = self_distillation_cfg.get("contrastive_visual_advantage", False)
            contrastive_negative_mode = self_distillation_cfg.get("contrastive_negative_mode", "no-image")
            contrastive_patch_mask_ratio = self_distillation_cfg.get("contrastive_patch_mask_ratio", 0.6)
            contrastive_patch_size = self_distillation_cfg.get("contrastive_patch_size", 14)
            post_encoder_visual_token_drop_rate = self_distillation_cfg.get(
                "post_encoder_visual_token_drop_rate", 0.3
            )
            input_visual_token_drop_rate = self_distillation_cfg.get("input_visual_token_drop_rate", 0.3)
            if teacher_extra_student_image_blocks == 0 and teacher_image_key not in batch.non_tensor_batch:
            ...
            for i in range(batch_size):
                ...
                raw_prompt_messages = list(batch.non_tensor_batch["raw_prompt"][i])
                if teacher_extra_student_image_blocks > 0:
                    student_images = self._extract_images_from_messages(raw_prompt_messages)
                    teacher_images = student_images * (teacher_extra_student_image_blocks + 1)
                else:
                ...
                if teacher_extra_student_image_blocks > 0 and teacher_prompt_messages is None:
                    teacher_messages = self._repeat_images_in_messages(
                        raw_prompt_messages,
                        teacher_extra_student_image_blocks,
                    )
                else:
                ...
                if contrastive_visual_advantage:
                    negative_teacher_messages = self._apply_contrastive_negative_to_messages(
                        teacher_messages,
                        contrastive_negative_mode,
                        patch_mask_ratio=contrastive_patch_mask_ratio,
                        patch_size=contrastive_patch_size,
                    )
                    (
                    ...
                    if contrastive_negative_mode == "post-encoder-token-drop":
                        negative_teacher_multi_modal_inputs["post_encoder_visual_token_drop_rate"] = torch.tensor(
                            [float(post_encoder_visual_token_drop_rate)],
                            dtype=torch.float32,
                        )
                    if contrastive_negative_mode == "input-visual-token-drop":
                        negative_teacher_multi_modal_inputs["input_visual_token_drop_rate"] = torch.tensor(
                            [float(input_visual_token_drop_rate)],
                            dtype=torch.float32,
                        )
            ...
            metrics = {
                ...
                "self_distillation/policy_fallback_fraction": (1.0 - teacher_present_mask.mean()).item(),
                "self_distillation/grpo_fallback_count": grpo_fallback_count,
                "self_distillation/contrastive_visual_advantage_enabled": float(contrastive_visual_advantage),
            }
            tensors = {
                "teacher_input_ids": teacher_input_ids,
                "teacher_attention_mask": teacher_attention_mask,
            ...
            if contrastive_visual_advantage:
                ...
                if negative_teacher_position_ids_list[0].dim() == 1:
                    ...
                    rope_dims = negative_teacher_position_ids_list[0].shape[0]
                    negative_teacher_position_ids = torch.zeros(
                        (batch_size, rope_dims, max_negative_teacher_len),
                        dtype=negative_teacher_position_ids_list[0].dtype,
                        device=device,
                    )
                    for i, position_ids in enumerate(negative_teacher_position_ids_list):
                ...
                tensors.update(
                    {
                        "negative_teacher_input_ids": negative_teacher_input_ids,
                        "negative_teacher_attention_mask": negative_teacher_attention_mask,
                        "negative_teacher_position_ids": negative_teacher_position_ids,
                        "negative_teacher_response_start_idx": torch.stack(
                            negative_teacher_response_start_idx_list
                        ).to(device),
                    }
                )
                non_tensors["negative_teacher_multi_modal_inputs"] = negative_teacher_multi_modal_inputs_list
